...
// --- FALLBACK CONDITIONS (all possible conditions) ---
const conditions = [
...
async function performSave() {
    ...
    const payload = {
        ...
        image: capturedImage   // <-- added image
    };
    ...
    try {
        const response = await fetch('/api/send-sms', {
            ...
            body: JSON.stringify({
                ...
                condition: lastResult.condition || lastResult.name,
                advice: lastResult.advice,
                severity: lastResult.severity   // <-- added severity
            })
        });
...
async function analyzeImage() {
    ...
    try {
        ...
        if (!allowedConditions.includes(aiResult.condition)) {
...
function displayResults(result) {
    lastResult = result;
    document.getElementById('result-image').src = capturedImage;
    ...
    // Severity badge (special case for "No issue detected")
...
// ---------- Hospital Screen Event Handlers ----------
